...
def get_loader(source_path, target_path, evaluation_path, transforms,
               batch_size=32, return_id=False, balanced=False, val=False, val_data=None, num_workers=4):
...
def get_loader_amlp(data_root, source_path, target_path, evaluation_path, transforms,
               batch_size=32, return_id=False, balanced=False, val=False, val_data=None,
               data_aug_crop=0.8, aug_type=0,num_workers=4
               ):
    ...
    source_folder = ImageFolder(data_root, os.path.join(source_path),
                                train_transform_sourse,
                                return_id=return_id)
    target_folder_train = ImageFolder(data_root, os.path.join(target_path),
                                  transform = train_transform_target,
                                  return_paths = False, return_id=return_id)
    if val:
        source_val_train = ImageFolder(data_root, val_data, train_transform_sourse, return_id=return_id)
        target_folder_train = torch.utils.data.ConcatDataset([target_folder_train, source_val_train])
        source_val_test = ImageFolder(data_root, val_data, transforms[evaluation_path], return_id=return_id)
    eval_folder_test = ImageFolder(data_root, os.path.join(evaluation_path),
                                   transform=transforms["eval"],
                                   return_paths=True)
    eval_folder_test_s = ImageFolder(data_root, os.path.join(source_path),
                                   transform=transforms["eval"],
                                   return_paths=True)
    if balanced:
        freq = Counter(source_folder.labels)
