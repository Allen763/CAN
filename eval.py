...
from utils.cluster_utils import evaluate_clustering
import os
import logging
...
def inference_amlp_v2_only_c2_maxcompair(dataset_test, G_mlp, Cs, class_list, entropy, thr, per_class_correct, \
                                         per_class_num, correct_close, correct, size, kno_num,\
                                         thr_open=0.5, sourse_loader_bool=False):
    ...
    for batch_idx, data in enumerate(dataset_test):
        with torch.no_grad():
            ...
            if entropy:
                ...
                ind_unk = torch.tensor(np.where(
                    np.max(out_open.data.cpu().numpy()[:, kno_num:], axis=1) >= \
                    np.max(out_open.data.cpu().numpy()[:, :kno_num], axis=1)
                    )[0])
            ...
            # gather clus_pred unk_lbl for clustering
            unk_mask = label_t >= open_class
            clus_pred.append(pred[unk_mask])
            clus_lbl.append(label_t[unk_mask])
            # change all open classes' label to "open_class"
            label_t[unk_mask] = open_class
            ...
            #         pred_all = np.r_[pred_all, out_t.data.cpu().numpy()]
            #         label_all = np.r_[label_all, label_t]
    label_all, pred_open, pred_ent, pred_all = [],[],[],[]
    clus_pred = torch.cat(clus_pred).cpu().numpy()
    clus_lbl = torch.cat(clus_lbl).cpu().numpy()
    return c1_softmax_all_predict_list, c2_softmax_all_predict_list, feature_list, predict_list, \
        path_list, pred_unk_list, label_all, pred_open, pred_ent, pred_all,\
              size, correct, correct_close, clus_pred, clus_lbl
...
def test_amlp_v2_only_c2_maxcompair(step, test_loader_s, dataset_test, name, n_share, G_mlp, Cs, mlp, kno_num,
         open_class = None, open=False, entropy=False, thr=0.5, argsSName=''):
    G, mlp = G_mlp
    G.eval()
    mlp.eval()
    ...
    (
        ...
        pred_unk_list, label_all, pred_open,
        pred_ent, pred_all, size,
        correct, correct_close, clus_pred, clus_lbl
    ) = inference_amlp_v2_only_c2_maxcompair(
        dataset_test, [G, mlp], Cs, class_list, entropy, thr,
        per_class_correct, per_class_num,
        correct_close, correct, size, kno_num, thr_open=0.5,
    )
    ...
    clus_acc, nmi, ari, pur = evaluate_clustering(clus_pred, clus_lbl)
    output = [f"step {step}",
              f"closed perclass", [round(i,3) for i in list(per_class_acc)],
              f"acc per class {(float(per_class_acc.mean())):.6f}",
              f"acc {float(acc_all):.6f}",
              f"acc close all {float(acc_close_all):.6f}",
              f"h score {float(h_score):.6f}",
              f"unk_: ", 
              f"clus_acc {float(clus_acc):.6f}",
              f"clus_nmi {float(nmi):.6f}",
              f"clus_ari {float(ari):.6f}",
              f"clus_pur {float(pur):.6f}",
            #   "roc %s"% float(roc),
            #   "roc ent %s"% float(roc_ent),
            #   "roc softmax %s"% float(roc_softmax),
            #   "best hscore %s"% float(best_acc),
    ...
    return acc_all, h_score, clus_acc, nmi, ari, pur 
