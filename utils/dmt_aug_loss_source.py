...
class MyLoss(nn.Module):
    def __init__(
        ...
        self.gamma_input = self._CalGamma(v_input) # coefficient
    ...
    def forward(self, input_data, input_data_aug, latent_data, latent_data_aug, rho, sigma):
        ...
        metaDistance_ = metaDistance.clone().detach()               # [128, 128]
        metaDistance_[torch.eye(metaDistance_.shape[0])==1.0] = metaDistance_.max()+1
        # 因为主对角线都是自己与自己的距离，因此是0.把对角线的数据换成整体的最大值，
        # 是为了在后续找最小值的时候找到除了自己以外的最小值（最近邻：nn,nearest neighbor）
        nndistance, _ = torch.min(metaDistance_, dim=0) #[128, 1]
        nndistance = nndistance / self.augNearRate
        downDistance = metaDistance + nndistance.reshape(-1, 1)  # [128, 128] + [128, 1]
        rightDistance = metaDistance + nndistance.reshape(1, -1) # [1, 128] + [128, 128]
